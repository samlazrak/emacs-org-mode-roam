...
(defun hp/obsidian-to-org--convert-frontmatter (content)
  "Convert YAML frontmatter in CONTENT to org properties."
  (if (string-match "^---\n\\(\\(?:.*\n\\)*?\\)---\n" content)
      (let* ((frontmatter (match-string 1 content))
        ...
        (with-temp-buffer
          ...
          (while (re-search-forward "^\\([^:]+\\):\\s-*\\(.+\\)$" nil t)
            (let ((key (match-string 1))
              ...
              ;; Handle YAML arrays properly: [tag1, tag2] -> "tag1 tag2"
              (when (string-match "\\[\\(.+\\)\\]" value)
                (setq value (match-string 1 value))
                (setq value (replace-regexp-in-string ",\\s-*" " " value))
                (setq value (replace-regexp-in-string "-\\s-*" "" value)))
              (push (cons (upcase (replace-regexp-in-string "-" "_" key)) value) properties))))

        ;; Build org properties drawer
        ...
        (dolist (prop (reverse properties))
          (let ((key (car prop))
            ...
            (cond
             ((string= key "TITLE")
              (setq org-content (concat org-content (format ":ROAM_ALIASES: %s\n" value))))
             ((string= key "TAGS")
              (setq org-content (concat org-content (format ":ROAM_TAGS: %s\n"
                                                           (string-trim (replace-regexp-in-string ",\\s-*" " " value))))))
             ((member key '("CREATED" "DATE" "LASTMODIFIED" "MODIFIED"))
              (setq org-content (concat org-content (format ":%s: %s\n" key value))))
             (t
              (setq org-content (concat org-content (format ":%s: %s\n" key value)))))))
        (setq org-content (concat org-content ":END:\n"))

        ;; Add title if present
        (when-let ((title (cdr (assoc "TITLE" properties))))
          (setq org-content (concat org-content (format "#+title: %s\n" title))))

        ;; Add filetags
        (when-let ((tags (cdr (assoc "TAGS" properties))))
          (let ((cleaned-tags (string-trim (replace-regexp-in-string ",\\s-*" " :" tags))))
...
(defun hp/obsidian-to-org--convert-buffer-content (content base-dir output-dir)
  "Convert Obsidian markdown CONTENT to org format.
BASE-DIR is the source directory, OUTPUT-DIR is the destination."
  (condition-case err
      (let ((result content))
        ;; Apply conversions step by step with error handling
        (condition-case sub-err
            (setq result (hp/obsidian-to-org--convert-frontmatter result))
          (error (message "Frontmatter conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (hp/obsidian-to-org--convert-wikilinks result))
          (error (message "Wikilinks conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (hp/obsidian-to-org--convert-image-embeds result base-dir output-dir))
          (error (message "Image conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (hp/obsidian-to-org--convert-tags result))
          (error (message "Tags conversion error: %s" (error-message-string sub-err))))
        
        ;; Text formatting conversions with fixed regexes
        (condition-case sub-err
            ;; Convert markdown headers to org headers: # Header → * Header
            ;; But NOT org directives like #+title: or #+filetags:
            (setq result (replace-regexp-in-string "^\\(#+\\)\\s-*\\([^+:].+\\)$" 
                                                   ...
                                                   result))
          (error (message "Headers conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (replace-regexp-in-string "\\*\\*\\([^*\n]+\\)\\*\\*" "*\\1*" result))
          (error (message "Bold conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (replace-regexp-in-string "__\\([^_\n]+\\)__" "*\\1*" result))
          (error (message "Bold alt conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (replace-regexp-in-string "\\b_\\([^_\n]+\\)_\\b" "/\\1/" result))
          (error (message "Italic conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (replace-regexp-in-string "~~\\([^~\n]+\\)~~" "+\\1+" result))
          (error (message "Strikethrough conversion error: %s" (error-message-string sub-err))))
        
        (condition-case sub-err
            (setq result (replace-regexp-in-string "==\\([^=\n]+\\)==" "@@html:<mark>@@\\1@@html:</mark>@@" result))
          (error (message "Highlight conversion error: %s" (error-message-string sub-err))))
        
        ;; Convert code blocks
        (condition-case sub-err
            (setq result (replace-regexp-in-string "```\\([a-zA-Z0-9-_+]*\\)\\s-*\n" "#+begin_src \\1\n" result))
          (error (message "Code block start conversion error: %s" (error-message-string sub-err))))
          
        (condition-case sub-err
            (setq result (replace-regexp-in-string "^```\\s-*$" "#+end_src" result))
          (error (message "Code block end conversion error: %s" (error-message-string sub-err))))
        
        ;; Convert inline code
        (condition-case sub-err
            (setq result (replace-regexp-in-string "`\\([^`\n]+\\)`" "~\\1~" result))
          (error (message "Inline code conversion error: %s" (error-message-string sub-err))))
        
        ;; Convert block quotes  
        (condition-case sub-err
            (setq result (replace-regexp-in-string "^> \\(.+\\)$" "#+begin_quote\n\\1\n#+end_quote" result))
          (error (message "Block quote conversion error: %s" (error-message-string sub-err))))
        
        ;; Convert single asterisk italic *text* to org italic /text/ (but not *[links]*)
        (condition-case sub-err
            (setq result (replace-regexp-in-string "\\*\\([^*\n\\[]+\\)\\*" "/\\1/" result))
          (error (message "Single asterisk italic conversion error: %s" (error-message-string sub-err))))
        
        ;; Convert markdown links [text](url) to org links [[url][text]]
        (condition-case sub-err
            (setq result (replace-regexp-in-string "\\[\\([^]]+\\)\\](\\([^)]+\\))" "[[\\2][\\1]]" result))
          (error (message "Markdown link conversion error: %s" (error-message-string sub-err))))
        
        result)
    (error
     (message "Critical error in content conversion: %s" (error-message-string err))
     content))) ; Return original content if conversion fails
...
(defun hp/obsidian-to-org-convert-buffer ()
  ...
  (let* ((input-file (buffer-file-name))
    ...
    ;; Convert content
    (let ((org-content (hp/obsidian-to-org--convert-buffer-content content base-dir output-dir)))
      ...
      (condition-case err
          (with-temp-buffer
            ...
            (let* ((base-dir (file-name-directory file))
              ...
              ;; Convert and save
              (let ((org-content (hp/obsidian-to-org--convert-buffer-content
                                content base-dir output-dir)))
                (with-temp-file output-file
                  (insert org-content)))))
        (error
